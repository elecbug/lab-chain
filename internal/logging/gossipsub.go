...
func (t *GossipsubTracer) Trace(evt *pb.TraceEvent) {
	...
	case *pb.TraceEvent_PUBLISH_MESSAGE.Enum():
		...
		log.Debugw(innerType, "messageID", hex.EncodeToString(evt.MessageID), "topic", *evt.Topic)
	...
	case *pb.TraceEvent_REJECT_MESSAGE.Enum():
		...
		log.Debugw(innerType, "messageID", hex.EncodeToString(evt.MessageID), "receivedFrom", peerID, "reason", *evt.Reason, "topic", *evt.Topic)
	...
	case *pb.TraceEvent_DUPLICATE_MESSAGE.Enum():
		...
		log.Debugw(innerType, "messageID", hex.EncodeToString(evt.MessageID), "receivedFrom", peerID, "topic", *evt.Topic)
	...
	case *pb.TraceEvent_ADD_PEER.Enum():
		...
		log.Debugw(innerType, "to", peerID, "proto", *evt.Proto)
	...
	case *pb.TraceEvent_REMOVE_PEER.Enum():
		...
		log.Debugw(innerType, "to", peerID)

	case *pb.TraceEvent_JOIN.Enum():
		evt := evt.Join

		log.Debugw(innerType, "topic", *evt.Topic)
	...
	case *pb.TraceEvent_GRAFT.Enum():
		...
		log.Debugw(innerType, "to", peerID, "topic", *evt.Topic)
